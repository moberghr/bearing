...
Squirrel is a cross-platform **Avalonia 12.1 desktop** SQL query tool & script manager for PostgreSQL
(.NET 10). Layered solution:

```
Squirrel.Core        abstractions + records only — zero dependencies
  ↑     ↑      ↑
Sql   Data   Persistence     (SQL parse/completion · Postgres via Npgsql · SQLite local state)
        ↑
Squirrel.App         Avalonia MVVM shell (ViewModels/Views/Controls/Input/Connections/Results)
        ↑
Squirrel.Desktop     thin entry point
```
...
- **SQL intelligence:** ANTLR4 PostgreSQL grammar + antlr4-c3 (`Squirrel.Sql`).
- **DI:** manual composition root in `src/Squirrel.App/App.axaml.cs` (no container).

## Build / test

- Build: `dotnet build`
- Test: `dotnet test` (Postgres integration tests skip cleanly with no DB; `SQUIRREL_TEST_PG_PORT=5434 dotnet test` to run live)
- Format: `dotnet format --verbosity quiet`

## Critical rules

1. **NEVER add a package/project reference to `Squirrel.Core`** — it stays dependency-free abstractions; everything depends on it, not the reverse.
...
- **Testing:** xUnit only (no NUnit/MSTest). No mocking library — use hand-rolled fakes (`tests/**/Fakes.cs`). Postgres tests use `Xunit.SkippableFact` + `SQUIRREL_TEST_PG_*`.
- **Input:** keyboard handling goes through the unified `src/Squirrel.App/Input/` pipeline (keymap + command registry); register a command, don't hand-roll `OnKeyDown`.

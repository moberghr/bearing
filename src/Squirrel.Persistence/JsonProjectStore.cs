...
public sealed class JsonProjectStore : IProjectStore
{
    ...
    public async Task<Project> OpenAsync(string directory, CancellationToken ct)
    {
        ...
        var manifest = await JsonSerializer.DeserializeAsync<ProjectManifest>(stream, SquirrelJson.Options, ct)
                       ?? new ProjectManifest();
    ...
    public async Task SaveAsync(Project project, CancellationToken ct)
    {
        ...
        await using (var stream = File.Create(tmp))
            await JsonSerializer.SerializeAsync(stream, project.Manifest, SquirrelJson.Options, ct);

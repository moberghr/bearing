...
/// keyed by attributes {app=squirrel, connection=&lt;guid&gt;} and never touch any file on disk.
/// </summary>
public sealed class SecretToolSecretStore : ISecretStore
{
    ...
    public async Task SetPasswordAsync(Guid connectionId, string password, CancellationToken ct)
    {
        var (exit, _, err) = await RunAsync(
            new[] { "store", "--label", $"Squirrel connection {connectionId}", "app", App, "connection", connectionId.ToString() },
            stdin: password, ct);
        if (exit != 0)

...
public sealed class FileRecentProjects : IRecentProjects
{
    ...
    public FileRecentProjects(string? path = null)
        => _path = path ?? Path.Combine(SquirrelPaths.ConfigDir, "recent.json");
    ...
    public async Task<IReadOnlyList<string>> ListAsync(CancellationToken ct)
    {
        ...
        return await JsonSerializer.DeserializeAsync<List<string>>(stream, SquirrelJson.Options, ct)
               ?? new List<string>();
    }
    ...
    public async Task AddAsync(string directory, CancellationToken ct)
    {
        ...
        await using (var stream = File.Create(tmp))
            await JsonSerializer.SerializeAsync(stream, list, SquirrelJson.Options, ct);

...
public sealed partial class ResultView
{
    ...
    private static bool IsBoolColumn(Bearing.Core.Data.ColumnDescriptor c)
    ...
    private static Control BuildColumnHeader(ResultSetViewModel result, int index)
    {
        ...
        if (result.PrimaryKeyColumns.Contains(index)) row.Children.Add(Badge("PK", "Accent.Brand"));
    ...
    private Control EditControls(ResultSetViewModel result, DataGrid grid)
    {
        ...
        var dot = new TextBlock { Text = "●", Foreground = Res("Accent.Brand"), VerticalAlignment = VerticalAlignment.Center, Margin = new Thickness(6, 0, 4, 0) };

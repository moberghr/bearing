# Pre-Commit Review List — Bearing
...
3. **`Bearing.Core` stays dependency-free** — no new `PackageReference`/`ProjectReference` added to Core (§2.1).
...
7. **Postgres tests are skip-safe** — new DB tests use `SkippableFact` + `BEARING_TEST_PG_*`, not plain `[Fact]` (§4.2).

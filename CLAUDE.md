...
- **Distribution:** Velopack installers + delta auto-update on this repo's GitHub Releases. **Cutting a release is publishing a GitHub Release** — `.github/workflows/release.yml` tests, builds both platforms and verifies the assets landed; the version is the git tag via MinVer, so nothing in the tree is bumped (§9.6a). See `docs/RELEASING.md`. `build/release.sh` is the older archive path, kept.

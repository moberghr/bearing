...
- **Layout:** `src/Bearing.{Core,Sql,Data,Persistence,Updates,Sessions,App,Desktop,Cli}` — dependency-free `Core` ← `Sql`/`Data`/`Persistence`/`Updates` ← `Sessions` ← `App` ← `Desktop`. `Sessions` is the windowless connect/credential/refusal layer a second host can reference (§2.6), and `Cli` is that second host — the `bearing` command: read-only access to exposed connections, and with no arguments it opens the app (the GUI apphost is `bearing-app`, §1.11)

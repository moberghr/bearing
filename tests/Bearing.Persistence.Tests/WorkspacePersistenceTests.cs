...
{
    private readonly string _root = Path.Combine(Path.GetTempPath(), "bearing-tests", Guid.NewGuid().ToString("N"));
    ...
    public async Task Session_side_pane_defaults_to_open_when_absent()
    {
        ...
        Directory.CreateDirectory(Path.Combine(dir, ".bearing"));
        // A legacy session.json predating the side-pane fields.
        await File.WriteAllTextAsync(Path.Combine(dir, ".bearing", "session.json"),
    ...
    public async Task Session_round_trips_open_editors_and_active_connection()
    {
        ...
        Assert.True(File.Exists(Path.Combine(dir, ".bearing", "session.json")));
